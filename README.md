...
Paste code, get a good-looking picture of it. Exports **PNG** and **SVG**, or
copies an inline-styled **HTML** snippet (selectable text) to the clipboard.
...
"Copy HTML" is the one exception: it reuses the same tokens and palette but
copies an HTML fragment to the clipboard — the window chrome wrapping a `<pre>`
of highlighted, still-selectable text, every style inlined — for pasting the
code segment straight into a blog post or CMS.
